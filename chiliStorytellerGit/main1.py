# same as main0.py but usuable with robot
#usuably with rR1_1.html
#usuable with the robot
import asyncio
import subprocess
import json
import os
...
class StorytellingApp:
    ...
    def robot_interaction(self): 
        ...
        if(local_data.split(' ', 1)[0]== "start"):
            ...
            self.conn.send(self.story_prompt)                
           
          #  print(self.story_prompt)
            self.robot_interaction()
        elif(local_data.split(' ', 1)[0]=="cQ1"):

            q1 = local_data.split(' ', 1)[1]

            
            print("Node started!\n")
            print("Send 'close' to finish session\n")

               # entry = input(questions_extracted)
            self.conn.send(q1)
            
           # print(questions_extracted)
            self.robot_interaction()  
        elif(local_data.split(' ', 1)[0]=="cQ2"):
            q2 = local_data.split(' ', 1)[1]
           ...
           # print(questions_extracted)
            self.robot_interaction()  
        elif(local_data.split(' ', 1)[0]=="cQ3"):
            q3 = local_data.split(' ', 1)[1]
        ...
        elif(local_data.split(' ', 1)[0]=="cA1"):

            a1= local_data.split(' ', 1)[1]
        ...
        elif(local_data.split(' ', 1)[0]=="cA2"):

            
            a2= local_data.split(' ', 1)[1]
        ...
        elif(local_data.split(' ', 1)[0]=="cA3"):
            a3= local_data.split(' ', 1)[1]           
           ...
           # print(questions_extracted)
            ...
            if(language == 'en'):
                message ="Thanks for listening! Hope you learnt a lot! See you next time"
            elif(language== 'fr'):
                message = "Merci d'avoir écouté ! J'espère que vous avez beaucoup appris ! À la prochaine fois"
            else:
                message = "Danke fürs Zuhören! Ich hoffe, Sie haben viel gelernt! Bis zum nächsten Mal."
...
def main():
    ...
    # Start the Node server in a new thread
    node_server_thread = threading.Thread(target=start_node_server)
    ...
    begin_html = os.path.abspath('/Users/NUR/nur_sm/index_files/chiliStorytellerGit/index.html')
